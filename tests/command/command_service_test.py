...
from mr_file_converter.file.errors import (FileConversionError,
    ...
    def test_error_handler_invalid_youtube_url_exception(
       ...
       """
        ...
        telegram_context.error = InvalidYouTubeURL(
            url='youtube.test',
            next_stage=YoutubeDownloaderService.check_youtube_url_stage
        )
        next_stage = command_service.error_handler(
            telegram_update, telegram_context
        )
        assert next_stage == YoutubeDownloaderService.check_youtube_url_stage
